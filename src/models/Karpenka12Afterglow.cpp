...
#include "Karpenka12Afterglow.hpp"
...
Karpenka12Afterglow::Karpenka12Afterglow() : Model() {
    noParams_ = 9;
    paramNames_ = {"A",
                   "B",
                   "t1",
...
double Karpenka12Afterglow::function(double t) {
    double flux = params_[0] * (1.0 + params_[1] * pow(t - params_[2], 2.0));
    flux *= exp(-(t-params_[5])/params_[4]);
    flux /= (1.0 + exp(-(t - params_[5]) / params_[3]));
    flux += (params_[7] * exp(-(t - params_[6])/params_[8]));
    return flux;
}


std::vector<double> Karpenka12Afterglow::function(std::vector<double>& t) {
...
std::vector<double> Karpenka12Afterglow::residual() {
...
int Karpenka12Afterglow::get_num_parameters() {
    return y_.size();
}
